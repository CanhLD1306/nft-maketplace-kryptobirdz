...
contract KBMarket is ReentraccyGuard {
    ...
    // charge a listing fee so the owner makes a comisstion
    ...
    public payable nonReentrant {
        // nonReetrant is a modifier to prevent reentry attack
    ...
    function fetchMarketTokens() public view returns (MarketToken[] memory) {
        ...
        // looping over the number of items create (if number has not been sold populate the array)
    ...
    function fetchMyNFTs() public view returns (MarketToken[] memory) {
        uint totalIteamCount = _tokenIds.current();
        //  a second counter for each individual user
        uint itemCount = 0;
        uint currentIndex = 0;

        for(uint i = 0; i < totalIteamCount; i++){
            if(idToMarketToken[i + 1].owner == msg.sender){
                itemCount + 1; 
            }
        }

        // second loop to loop through the amount you have purchase with itemCount
        // check to see id the owner address is equal to msg.sender
    ...
    function fetchItemsCreate() public view returns (MarketToken[] memory) {
